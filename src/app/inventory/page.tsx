...
const Table = ({
  ...
  <table
    className={`w-full border border-gray-800 text-left text-white ${className}`}
  >
    {children}
  </table>
);
const TableHead = ({
  ...
  <thead className={` bg-purple-500/30 backdrop-blur-md ${className}`}>
    {children}
  </thead>
);
const TableRow = ({
...
const InventoryPage = () => {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      const fetchedCategories = res.data.data.map(
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await axios.put(
        `/api/v1/product/?id=${selectedProduct.id}`,
        editFormData
      );
            ...
            <div className="bg-gray-400/10 bg-opacity-30 backdrop-blur-md text-white rounded-xl border border-white/20 shadow-lg w-96 p-6">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1 bg-purple-500 text-white rounded hover:bg-purple-700 focus:outline-none"
                >
                  Save
                </button>
              </div>
